...
auto solve(std::string_view input) -> int64_t {
  auto packets = std::vector<std::string_view>{};
  packets.reserve(1024);  // NOLINT
  ...
  while (true) {
    ...
    packets.emplace_back(packet_1_begin, size_t(std::distance(packet_1_begin, packet_1_end)));
    packets.emplace_back(packet_2_begin, size_t(std::distance(packet_2_begin, packet_2_end)));
  ...
  static constexpr auto additional_packet_1 = std::string_view("[[2]]");
  static constexpr auto additional_packet_2 = std::string_view("[[6]]");
  packets.push_back(additional_packet_1);
  packets.push_back(additional_packet_2);
  ranges::sort(packets, compare);
  
  auto additional_packet_1_pos = ranges::find(packets, additional_packet_1);
  auto additional_packet_2_pos = ranges::find(packets, additional_packet_2);

  return (std::distance(packets.begin(), additional_packet_1_pos) + 1) *
         (std::distance(packets.begin(), additional_packet_2_pos) + 1);
}

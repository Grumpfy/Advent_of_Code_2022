...
auto solve(std::string_view input) -> int64_t {
  ...
  auto const max_y = ranges::max(
      rocks | ranges::views::join | ranges::views::transform(&Pos2D::second)) + 2;
  auto const min_y = 0;
  auto const sand_emitter_pos = Pos2D{500,0};
  auto const min_x = sand_emitter_pos.first - max_y - 10;
  auto const max_x = sand_emitter_pos.first + max_y + 10;
  rocks.emplace_back();
  rocks.back().emplace_back(min_x, max_y);
  rocks.back().emplace_back(max_x, max_y);

  auto const x_offset = -min_x;
  auto const y_offset = -min_y;
  auto const width = max_x - min_x + 1;
  auto const height = max_y - min_y + 1;
  auto index = [&](Pos2D const &pos) -> size_t {

...
auto solve(std::string_view input) -> std::string {
  ...
  for (auto [match, crate_number, src_stack, dst_stack] :
    ...
    auto & src = stacks.at(src_stack.to_number<size_t>() - 1U);
    auto & dst = stacks.at(dst_stack.to_number<size_t>() - 1U);
    auto const count = crate_number.to_number<size_t>();
    for (auto crate_count = size_t{}; crate_count < count; ++crate_count) {
      dst.push_back(src.at(src.size()-count+crate_count));
    }
